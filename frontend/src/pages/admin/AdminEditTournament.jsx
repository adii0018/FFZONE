/**
 * FFZone – Admin Edit Tournament Page
 */
import { useState, useEffect } from 'react'
import { useNavigate, useParams } from 'react-router-dom'
import { useDropzone } from 'react-dropzone'
import { FiUpload, FiCheckCircle, FiArrowLeft, FiSave } from 'react-icons/fi'
...
export default function AdminEditTournament() {
  const { id } = useParams()
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      await api.patch(`/tournaments/${id}/update/`, fd, { 
            ...
            <div {...getRootProps()} className={`border-2 border-dashed rounded-xl p-8 text-center cursor-pointer transition-all ${
              ...
              {banner ? (
                ...
                <div>
                  ...
                  <p className="text-white/20 text-xs mt-1">Leave empty to keep current banner</p>
                </div>
              )}
            </div>
          </div>
